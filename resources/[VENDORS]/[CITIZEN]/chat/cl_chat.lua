...
local debug_print = GetConvar("chat_server_message", "false") == "true";
...
RegisterCommand("open_citizen_chat", function()

	local pause_menu_active = IsPauseMenuActive();

	if (not chatInputActive and not pause_menu_active) then
...
RegisterKeyMapping('open_citizen_chat', 'Open Chat', 'keyboard', 'T');
