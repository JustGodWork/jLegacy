...
if Config.EnableDebug then
    ...
    AddEventHandler('esx:playerPedChanged', function(netId)
        console.warn('esx:playerPedChanged', source, netId)
    end)

    AddEventHandler('esx:playerJumping', function()
        console.warn('esx:playerJumping', source)
    end)

    AddEventHandler('esx:enteringVehicle', function(plate, seat, netId)
        console.warn('esx:enteringVehicle', 'source', source, 'plate', plate, 'seat', seat, 'netId', netId)
    end)

    AddEventHandler('esx:enteringVehicleAborted', function()
        console.warn('esx:enteringVehicleAborted', source)
    end)

    AddEventHandler('esx:enteredVehicle', function(plate, seat, displayName, netId)
        console.warn('esx:enteredVehicle', 'source', source, 'plate', plate, 'seat', seat, 'displayName', displayName, 'netId', netId)
    end)

    AddEventHandler('esx:exitedVehicle', function(plate, seat, displayName, netId)
        console.warn('esx:exitedVehicle', 'source', source, 'plate', plate, 'seat', seat, 'displayName', displayName, 'netId', netId)
    end)
