...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, job2, loadout, name, coords, metadata)
	...
	if Config.Multichar then self.license = Config.DefaultIdentifier .. identifier:sub(identifier:find(':'), identifier:len()) else self.license = ('%s:%s'):format(Config.DefaultIdentifier, identifier) end
	...
	function self.setAccountMoney(accountName, money, reason)
		...
		if not tonumber(money) then 
			console.err(('Tried To Set Account ^5%s^0 For Player ^5%s^0 To An Invalid Number -> ^5%s^7'):format(accountName, self.playerId, money))
			return
		end
		if money >= 0 then
			...
			if account then
				...
				console.err(('Tried To Set Invalid Account ^5%s^0 For Player ^5%s^0!'):format(accountName, self.playerId))
			end
		else 
			console.err(('Tried To Set Account ^5%s^0 For Player ^5%s^0 To An Invalid Number -> ^5%s^7'):format(accountName, self.playerId, money))
		end
	...
	function self.addAccountMoney(accountName, money, reason)
		...
		if money > 0 then
			...
			if account then
				...
				console.err(('Tried To Set Add To Invalid Account ^5%s^0 For Player ^5%s^0!'):format(accountName, self.playerId))
			end
		else 
			console.err(('Tried To Set Account ^5%s^0 For Player ^5%s^0 To An Invalid Number -> ^5%s^7'):format(accountName, self.playerId, money))
		end
	...
	function self.removeInventoryItem(name, count, metadata, slot)
		...
		if item then
			...
			if count > 0 then
				...
				console.err(('Player ID:^5%s Tried remove a Invalid count -> %s of %s'):format(self.playerId, count,name))
			end
		end
	end
	...
	function self.canCarryItem(name, count, metadata)
        if ESX.Items[name] then
            ...
            console.warn(('Item ^5"%s"^7 was used but does not exist!'):format(name))
        end
	end
	...
	function self.setJobByType(jobType, value, grade)

		if (not self[jobType]) then return; end

		grade = tostring(grade);
		local lastJob = json.decode(json.encode(self[jobType]));

		if ESX.DoesJobExist(value, grade) then
			local jobObject, gradeObject = ESX.Jobs[value], ESX.Jobs[value].grades[grade]

			self[jobType].id    = jobObject.id;
			self[jobType].name  = jobObject.name;
			self[jobType].label = jobObject.label;

			self[jobType].grade        = tonumber(grade);
			self[jobType].grade_name   = gradeObject.name;
			self[jobType].grade_label  = gradeObject.label;
			self[jobType].grade_salary = gradeObject.salary;

			if (gradeObject.skin_male) then
				self[jobType].skin_male = json.decode(gradeObject.skin_male);
			else
				self[jobType].skin_male = {};
			end

			if (gradeObject.skin_female) then
				self[jobType].skin_female = json.decode(gradeObject.skin_female);
			else
				self[jobType].skin_female = {};
			end
			...
			console.warn(('Ignoring invalid ^5.%s()^7 usage for ID: ^5%s^7, Job: ^5%s^7'):format(jobType == "job" and "setJob" or "setJob2", self.source, value));
		end
	...
	function self.setJob(job, grade)
		self.setJobByType("job", job, grade);
	end

	---@param job2 string
	---@param grade number | string
	function self.setJob2(job2, grade)
		self.setJobByType("job2", job2, grade);
	end
	...
	function self.getMeta(index, subIndex)
		if index then
			...
			if type(index) ~= "string" then
				return console.err("xPlayer.getMeta ^5index^7 should be ^5string^7!")
			end
			...
			if self.metadata[index] then
				...
				return console.err(("xPlayer.getMeta ^5%s^7 not exist!"):format(index))
			end
	...
	function self.setMeta(index, value, subValue)
		if not index then
			return console.err("xPlayer.setMeta ^5index^7 is Missing!")
		end

		if type(index) ~= "string" then
			return console.err("xPlayer.setMeta ^5index^7 should be ^5string^7!")
		end

		if not value then
			return console.err(("xPlayer.setMeta ^5%s^7 is Missing!"):format(value))
		end
		...
		if not subValue then
			...
			if _type ~= "number" and _type ~= "string" and _type ~= "table" then
				return console.err(("xPlayer.setMeta ^5%s^7 should be ^5number^7 or ^5string^7 or ^5table^7!"):format(value))
			end
			...
			if _type ~= "string" then
				return console.err(("xPlayer.setMeta ^5value^7 should be ^5string^7 as a subIndex!"):format(value))
			end
	...
	function self.clearMeta(index)
		if not index then
			return console.err(("xPlayer.clearMeta ^5%s^7 is Missing!"):format(index))
		end
		...
		if not self.metadata[index] then
			return console.err(("xPlayer.clearMeta ^5%s^7 not exist!"):format(index))
		end
