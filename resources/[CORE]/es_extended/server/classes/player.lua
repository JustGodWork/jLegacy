...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, job2, loadout, name, coords, metadata)
    ...
    if Config.Multichar then
        self.license = Config.DefaultIdentifier .. identifier:sub(identifier:find(":"), identifier:len())
    else
        self.license = ('%s:%s'):format(Config.DefaultIdentifier, identifier)
    end
    ...
    function self.setJob(newJob, grade)
        self.setJobByType("job", newJob, grade)
    end
    ...
    function self.setJob2(newJob, grade)
        self.setJobByType("job2", newJob, grade)
    end
